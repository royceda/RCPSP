...
void Flow::solve(Parser& p) {
	...
	constraints.add(ex == p.resAvail()[k]);
	ex.end();
	...
	constraints.add(en == p.resAvail()[k]);
	en.end();            
      }
	...
	for (unsigned int j = 1; j < p.jobs(); j++) {
	  for (unsigned int k = 0; k < p.nOfRes(); k++) {
	    if (p.reqJobsMach()[i][k] < p.reqJobsMach()[j][k])
	      ...
	      //constraints.add(f[i][j][k] <= (x[i][j] * p.reqJobsMach()[i][k]));
	    else
	      model.add(f[i][j][k] <= (x[i][j] * p.reqJobsMach()[j][k]));
	      //constraints.add(f[i][j][k] <= (x[i][j] * p.reqJobsMach()[j][k]));
	  }
	}
      }
